...
Processor.prototype.import = function(file, done) {
  ...
  var mapFile = _.get(self._options, "map") || "";
  var attrsMap = {};

  if (!_.isEmpty(mapFile)) {
    try {
      var absMapFile = path.resolve(mapFile);
      attrsMap = require(absMapFile);
    } catch (e) {
      console.log("Unable to load map file: '%s'", mapFile);
    }
  }
  ...
  function handler(srcFile, lineNo, line, next) {
    ...
    if (_.isEmpty(attrs)) return next();
    ...
    _.set(attrs, "sf", (inLocalDir ? relSrcFile : srcFile));
    _.set(attrs, "sl", lineNo);

    var mappedAttrs = mapAttributes(attrs, attrsMap);

    var query = toSqlInsert(destTable, _.omit(mappedAttrs, toIgnore));
...
function mapAttributes(obj, map) {
  //-- return original obj if there's no mapping
  if (_.isEmpty(map)) {
    return obj;
  }

  var mapping = _.omit(map, "*");
  var rest = _.get(map, "*", "");

  var newObj = _.reduce(mapping, function(out, val, key) {
    out[key] = _.get(obj, val, "");
    return out;
  }, {});
  ...
  switch (rest) {
    case "*":
      ...
      break;
    case "_":
      _.merge(newObj, _.omit(obj, _.values(mapping)))
      break;
  }
